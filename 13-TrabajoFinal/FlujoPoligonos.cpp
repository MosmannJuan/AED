...
bool EnviarPoligono(std::ostream& os, const Poligono& pol){
    ...
    for(auto aux = pol.PrimerPunto; aux; aux = aux->SigNodo) EnviarPunto(os, aux->p);
    os << "# " <<"\n" ;
    return bool (os);
}
...
void MostrarLista(const ListaPoligonos& lista){
    std::cout << "Los Poligonos de la lista son: " <<"\n";
...
void AddPoligono(ListaPoligonos& lista, const Poligono& pol){
    ...
    n->Pol.c.g = pol.c.g;
    n->Pol.c.b = pol.c.b;
    n->Pol.PrimerPunto = pol.PrimerPunto;
